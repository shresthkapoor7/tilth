...
export class Encounters{
 constructor({actors=ACTORS,mover=advance,onProvoked=()=>{},onFeedback=()=>{},onImpact=()=>{},onDefeated=()=>{}}={}){this.actors=actors.map(a=>({...a,maxHp:a.hp,home:{x:a.x,y:a.y},direction:'down',hostile:false,attack:null,ready:0,downUntil:0,peaceAt:0,nextRoam:0,walk:false,bubble:'',bubbleUntil:0,revision:0}));this.mover=mover;this.onProvoked=onProvoked;this.onFeedback=onFeedback;this.onDefeated=onDefeated;this.onImpact=onImpact;this.time=0;this.hp=280;this.maxHp=280;this.swing=null;this.invulnerable=0;this.guardUntil=0;this.potionReady=0;this.floats=[]}
 ...
 hit(a,damage){if(a.attackable===false)return;const first=!a.hostile;a.hp=Math.max(0,a.hp-damage);this.onImpact(a.x,a.y,damage);a.hostile=true;a.peaceAt=this.time+14000;this.floats.push({x:a.x,y:a.y,text:`−${damage}`,until:this.time+900});if(first)a.revision++;if(this.time-(a.lastReaction??-Infinity)>30000){a.lastReaction=this.time;this.say(a,a.warning);this.onProvoked(a)}if(a.hp===0){a.downUntil=this.time+18000;a.hostile=false;a.attack=null;a.revision++;this.say(a,a.enemy?'Defeated':'Enough… I yield.');this.onFeedback(`${a.name} ${a.enemy?'defeated':'yielded'}.`);this.onDefeated(a);}}
 ...
 for(const a of this.actors){a.walk=false;if(a.downUntil){if(a.persistentDefeat)continue;if(now>=a.downUntil){a.downUntil=0;a.hp=a.maxHp;a.x=a.home.x;a.y=a.home.y;a.ready=now+2000;a.revision++}continue}
 const distance=Math.hypot(a.x-player.x,a.y-player.y),strike=attackStats(a.weapon,'Heavy');
 if(a.enemy&&!a.hostile&&distance<(a.relentless?155:95)&&now>=a.ready&&clearReach(a,player,this.mover)){a.hostile=true;a.peaceAt=now+14000;this.say(a,a.warning||'This road belongs to us!')}
 ...
 if(a.attack){const elapsed=now-a.attack.start;if(!a.attack.hit&&elapsed>=500){a.attack.hit=true;if(now>=this.invulnerable&&inStrike(a,player,a.attack.direction,strike,this.mover)){const damage=now<this.guardUntil?3:a.enemy?13:9;this.hp=Math.max(0,this.hp-damage);this.onImpact(player.x,player.y,damage);this.invulnerable=now+650;this.floats.push({x:player.x,y:player.y,text:`−${damage}`,until:now+900});this.onFeedback(`${a.name} hit you for ${damage}.`)}}if(elapsed>=850)a.attack=null;continue}
 ...
 if(this.hp<=0){this.hp=this.maxHp;player.x=400;player.y=335;this.invulnerable=now+3500;this.swing=null;this.generated=null;for(const a of this.actors){a.hostile=false;a.attack=null;a.ready=now+5000;a.revision++}this.onFeedback('You were overwhelmed. You recover on safe ground.');}
 }
}
