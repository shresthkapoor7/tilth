import {taskInstruction} from './regional-tasks.js';
import {regionLife,regionProgress,acceptRegionalQuest,regionalTasks,regionalTaskDone,regionalReady} from './region-life.js';
...
export function installGameUI({runtime,queue,modal,body,title,redraw,onAccept,onUse}){
 ...
 function render(){if(!view)return;const state=runtime.state;
  ...
  if(view==='quests'){
   title.textContent='Your quests';body.innerHTML='<p class="modal-note">A promise is yours to accept. Follow the marker, complete each task, then report back.</p>'+[...state.quests].sort((a,b)=>(['offered','active'].includes(b.status)?1:0)-(['offered','active'].includes(a.status)?1:0)).map(q=>`<div class="journal-entry"><h2>${escape(q.content.title)}</h2><p>${escape(q.content.description)}</p>${q.content.objectives.map((o,i)=>`<p>${q.completed[i]?'✓':'◇'} ${escape(o.description)}</p>`).join('')}<p>${escape(q.status)}</p>${['active','offered'].includes(q.status)?`<button class="outline" data-replace-quest="${escape(q.id)}">Request a different task</button>`:''}${q.status==='offered'?`<button class="primary" data-quest="${escape(q.id)}">Accept quest</button>`:''}</div>`).join('');body.querySelectorAll('[data-quest]').forEach(b=>b.onclick=()=>runtime.acceptQuest(b.dataset.quest));body.querySelectorAll('[data-replace-quest]').forEach(b=>b.onclick=()=>runtime.replaceQuest(b.dataset.replaceQuest));
  }
  if(view==='quests'){
   const cards=Object.entries(state.regions||{}).map(([id,entry])=>{const p=regionProgress(entry),life=regionLife(entry.content);if(!p.offered)return '';const ready=regionalReady(entry);return `<div class="journal-entry"><small>${escape(entry.content.name)} · Resident request</small><h2>${escape(life.objective.title)}</h2><p>${escape(life.objective.description)}</p>${regionalTasks(entry.content).map(t=>`<p>${regionalTaskDone(entry,t)?'✓':'◇'} ${escape(taskInstruction(entry,t))}</p>`).join('')}<p>${p.rewarded?'✓':'◇'} Report to a resident inside a house.</p><p>Reward: health restored and one awakening offer.</p><p>${p.rewarded?'complete':p.accepted?(ready?'Ready to report back':'active'):'offered'}</p>${!p.accepted?`<p>You can decide later. Enemies already defeated count toward this task.</p><button class="primary" data-region-quest="${escape(id)}">Accept quest</button>`:''}</div>`}).join('');
