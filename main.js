...
landscape();
function draw(){canvas.style.objectPosition=`${player.x/800*100}% ${player.y/600*100}%`;ctx.drawImage(currentRoom?roomCanvas:atlas.content?regionCanvas:bg,0,0);if(!currentRoom){for(const actor of encounters.actors)drawEncounter(ctx,actor,encounters.time);if(!atlas.content&&!rowan.hostile&&!rowan.downUntil)pixelText(ctx,'!',rowan.x,rowan.y-31,'#ffec98',23);}if(currentRoom&&regionalHouse)drawHero(ctx,510,280,'#a79665',1,regionalHouse.resident,false,{role:'Healer',direction:'down'});sprite(ctx,player.x,player.y,gear==='Emberweave cloak'?'#b26943':colors[heroClass],1.2,'Evergreen');const awakened=runtime.activeOffer()?.content;if(awakened&&activeGeneratedMove)drawAwakenedMove(ctx,player.x,player.y,activeGeneratedMove,(performance.now()-generatedSkillStart)/awakened.skill.durationMs,generatedDirection);const guidance=worldGuidance();if(guidance.target){const {x,y}=guidance.target;pixelText(ctx,'▼',x,y-35,'#ffe2a0',19);rect(ctx,x-10,y+12,20,2,'#edc67a');}if(!currentRoom&&!atlas.content)atmosphere(ctx,performance.now()/1000);rect(ctx,player.x-12,player.y+19,25,3,'#172325');rect(ctx,player.x-11,player.y+19,22*encounters.hp/encounters.maxHp,2,'#d9df92');if(!currentRoom){drawEncounterSpeech(ctx,encounters.actors,encounters.time);for(const f of encounters.floats)pixelText(ctx,f.text,f.x,f.y-35-(900-f.until+encounters.time)/35,'#ffb383',19);}if(frame>0){pixelText(ctx,frameText,player.x,player.y-52-(60-frame)/4,'#fff4b0',20);frame--}}let frameText='';draw();
...
function move(dir){
 if(modal.open||dialogue.active||(creator?.dialog.open||witch?.dialog.open)||performance.now()-attackStart<activeDuration())return;
