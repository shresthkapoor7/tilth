...
- [Playable illustrated inn](docs/ART_STUDY.md) now uses Tilth-specific bed/table/chest and Ember drawings, the original room palette, exact furnishing footprints and a prepared directional atlas. The first generated drawings were preserved; approved background removal changes alpha only. The earlier angled desk/cupboard and blue-haired outfit are no longer loaded. All 47 tests and the documented build pass. Browser checks cover comparison, movement and editor fallback; the earlier proof verified a bow combo and normal exit.
- [Character art architecture](docs/CHARACTER_ART.md) describes registered parts and color masks for the existing editor. The current single appearance defaults to saved pixel rendering for unsupported hairstyles, colors or clothes. Separate Drawn character / Pixel character buttons now allow an explicitly labeled sample preview for any profile, with a larger character thumbnail and no save changes. Browser regression verifies the controls on desktop and narrow layouts. Full modular drawn customization is not implemented yet.
