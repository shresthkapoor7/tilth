...
function rock(c,x,y,s=1,scene){scene?.push({asset:'rock',x:x-27*s,y:y-51*s,w:52*s,h:58*s});addObstacle(x-22*s,y-15*s,44*s,23*s);c.save();c.translate(x,y);c.scale(s,s);poly(c,[[-24,4],[-27,-11],[-16,-36],[-4,-51],[13,-43],[25,-15],[23,7]],'#151b1c');poly(c,[[-23,-11],[-14,-34],[-4,-47],[1,-15],[-5,2]],'#424341');poly(c,[[-4,-47],[11,-40],[21,-13],[1,-15]],'#343837');poly(c,[[1,-15],[21,-13],[20,4],[-5,2]],'#272e2e');R(c,-12,-33,3,12,'#646055');R(c,-16,-17,8,2,'#514f48');R(c,8,-10,2,8,'#935340');R(c,9,-10,7,2,'#b2643d');c.restore()}
function deadTree(c,x,y,s=1,scene){scene?.push({asset:'tree',x:x-20*s,y:y-81*s,w:37*s,h:86*s});addObstacle(x-7*s,y-8*s,14*s,14*s);c.save();c.translate(x,y);c.scale(s,s);poly(c,[[-12,5],[-4,-10],[-5,-32],[-17,-43],[-20,-61],[-15,-49],[-4,-41],[-3,-72],[3,-81],[1,-43],[11,-51],[17,-72],[17,-47],[5,-31],[5,-9],[15,5]],'#111b1c');R(c,-1,-38,3,37,'#51443a');R(c,1,-34,1,13,'#8b5138');R(c,-15,-45,7,2,'#594d40');c.restore()}
...
// Drawing registration is separate from collision; both styles reuse this authored layout.
scene.push({asset:'bridge',x:567,y:447,w:155,h:73});
...
houses.forEach(h=>fortress(c,h.x,h.y,h.w,scene,h.id));
...
scene.push({asset:'forge',x:446,y:296,w:51,h:36},{asset:'outdoor-anvil',x:432,y:322,w:32,h:23});
scene.push(...[{x:84,y:246,w:79,h:29},{x:199,y:246,w:89,h:29},{x:75,y:390,w:76,h:25},{x:189,y:390,w:92,h:25}].map(p=>({asset:'fence',...p})));
...
for(let x=474;x<565;x+=21){if(x>512&&x<556)continue;scene.push({asset:'crate',x,y:238,w:16,h:15});R(c,x,238,16,15,'#242723');R(c,x+1,239,14,12,'#69523a');R(c,x+3,241,10,8,'#443d2d');R(c,x+7,239,2,13,'#ad8852')}
...
for(let x=0;x<800;x+=31){if(x<309||x>424)rock(c,x,50+rand()*24,.8+rand()*.8,scene);if(x<308)rock(c,x,615+rand()*8,1+rand()*.4,scene)}for(let y=120;y<570;y+=53)rock(c,10,y,1.4,scene);
for(let [x,y,s] of [[72,160,1],[277,153,.75],[552,418,.6],[293,493,.9],[65,466,1],[755,283,1.8],[756,574,2],[748,131,1.4]])rock(c,x,y,s,scene);
for(let [x,y,s] of [[65,238,1],[284,224,.7],[60,553,1.2],[512,564,.9],[560,94,.8],[229,77,.7]])deadTree(c,x,y,s,scene);
for(let [x,y] of [[307,272],[425,272],[305,384],[426,384],[571,442],[571,528],[256,209],[500,209]])brazier(c,x,y,scene);
